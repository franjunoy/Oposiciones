...
const WebsiteInput = ({ setValue }) => {
    ...
    useEffect(() => {    
        if (isValid) {
            ...
            setError('Enter a correct website')          
            setDivBlue(false)
            setValue('')
        }
    },[isValid, website])
     ...
     const onChange = (e) => {    
        ...
        setIsValid(validateWebsite(newWebsite));    
    }

  return (
    <div className="flex flex-col w-80 items-start gap-1.5">        

        <label className="w-full h-full leading-5 font-primary text-base font-medium">Website</label>

        <div className="flex w-full h-full">
            <div className="flex px-3.5 py-2.5 items-center border rounded  border-gray-300">
                <p className="text-gray-600 font-primary text-base font-normal leading-6">http://</p>
            </div>

            <div className={`flex w-full h-full px-3.5 py-2.5 bg-white border rounded items-center gap-2  ${divBlue ? "border-blue-300" : ''} ${error ? "border-red-300" : website ? 'border-green-300' : ''} `}>
                <input
                    type='text'
                    value={website}
                    onChange={((e) => onChange(e))}
                    placeholder='www.ey.com'
