...
const NumberInput = ({
  ...
  const onChange = (e) => {
    ...
    // Verificar si el campo está vacío o contiene solo ceros
    if (inputValue === "" || parseInt(inputValue) === 0) {
      // En este caso, establecer el estado 'number' a una cadena vacía
      setNumber("");
      setIsValid(true); // Opcional: establecer como válido
    } else {
      // Si no es un campo vacío o ceros, actualizar el estado 'number'
  ...
  return (
    <div className={`flex flex-col ${weight} items-start`}>
      <label className="w-10 h-5 font-primary">{label}</label>
      <div
        className={`flex w-full h-11 px-3.5 py-2.5 bg-white border rounded items-center border-gray-300   ${
          divBlue ? "border-blue-300" : ""
        } ${error ? "border-red-300" : number ? "border-green-300" : ""}`}>
        <input
          type="number"
          value={number}
      ...
      {error ? (
        ...
        <span
          className={`w-full h-4 text-gray-500 text-sm font-light leading-6 font-primary  ${visible}`}>
          {hint}
        </span>
      )}
    </div>
